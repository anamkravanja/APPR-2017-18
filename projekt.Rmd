...
Najnižjo povprečno plačo so prejemali zaposleni v gostinjstvu.
...
Sedaj lahko podatke nadgradimo in pogledamo primerjavo še glede na spol in razporeditev po letih. Neglede na izobrazbo in sektor imajo moški v povprečju višje plače kot ženske. Vidimo lahko, da je porazdelitev bruto plač izredno asimetrična. Mediana mesečne bruto plače deli populacijo na dva dela. Spodaj so tisti, ki imajo osnovnošolsko in srednješolsko izobrazbo, nad njo pa tisti z višje in visokošilsko izobrazbo. Neglede na sektor je plača pri moških v povprečju višja od plače žensk.  
...
print(graf3)
```
Vidimo lahko, da se linearni model ne prilega najbolje samim točkam. Bolj ustreza polinom 3. stopnje (krivulja turkizne barve) oziroma optimalen model lokalne regresije (krivulja oranžne barve). Zopet lahko opazimo deljenje v dve skupini. Zgostitev je pri nižjih plačah in nižjih izdatkih, oziroma pri višjih plačah in višjih izdatkih. Nekoliko nas preseneti mogoče to, da bi pričakovali da se z višino minimalne plače višajo tudi izdatki, ki jih namenijo ljudje za potovanje, pa ni čisto tako. Tukaj bi bilo potrebno upoštevati tudi razmere v sami državi in delež državljanov, ki prejemajo minimalne plače. 

ZAKLJUČEK
Če pogledamo trende, ki nam jih nakazujejo podatki, je najbolj očitno to, da se bodo plače višale. Težko je podati kakšne koli napovedi, saj je veliko parametrov, ki jih nismo upoštevali.
Ugotovili pa smo, kej so plače najvišje in kje najnižje. Mislim, da je to podobno predvidevanjim, da so najvišje plače v najbolj razvitem delu Slovenije. Pregled podatkov je potrdil naša predvidevanja, ki pa niso temeljila na statističnih podatkih. 
